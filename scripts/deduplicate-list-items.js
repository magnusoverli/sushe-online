...
async function deduplicateListItems() {
  ...
  console.log('Strategy: NULL = "use albums table", value = "custom override"\n');
  ...
  try {
    ...
    for (const row of res.rows) {
      ...
      if (row.cover_image && valuesMatch(row.cover_image, row.album_cover_image)) {
      ...
      if (row.cover_image_format && valuesMatch(row.cover_image_format, row.album_cover_image_format)) {
      ...
      if (row.release_date && valuesMatch(row.release_date, row.album_release_date)) {
      ...
      if (row.tracks && row.album_tracks) {
        try {
          const tracksEqual = JSON.stringify(row.tracks) === JSON.stringify(row.album_tracks);
          if (tracksEqual) {
            updates.push('tracks = NULL');
            stats.tracksNulled++;
          }
        } catch (err) {
          console.error(`  ⚠️  Error comparing tracks for item ${row._id}:`, err.message);
          stats.errors++;
        }
      }
      ...
      processed++;
      if (processed % 50 === 0) {
        console.log(`  Progress: ${processed}/${stats.totalItems} items processed...`);
      }
    }
    ...
    const totalFieldsNulled = 
      stats.coverImageNulled + stats.coverFormatNulled +
      stats.artistNulled + stats.albumNulled +
      stats.releaseDateNulled + stats.countryNulled +
      stats.genre1Nulled + stats.genre2Nulled + stats.tracksNulled;
    ...
    console.log(`  Items with custom covers: ${verifyRes.rows[0].items_with_cover}`);
    console.log(`  Items using albums table cover: ${verifyRes.rows[0].items_using_album_cover}\n`);
