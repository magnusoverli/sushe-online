...
class PlanningUtils {
  ...
  async createPlanFromTemplate(planData) {
    ...
    let content = template
      ...
      .replace(/\[Development\/Infrastructure\/Research\/Bug Fix\/Feature\]/g, planData.type || 'Development')
      .replace(/\[Detailed description of what this plan aims to accomplish\]/g, planData.description || 'Plan description to be added.')
      .replace(/\[Primary objective 1\]/g, planData.objectives?.[0] || 'Objective 1 to be defined')
      .replace(/\[Primary objective 2\]/g, planData.objectives?.[1] || 'Objective 2 to be defined')
      .replace(/\[Primary objective 3\]/g, planData.objectives?.[2] || 'Objective 3 to be defined');
    ...
    if (planData.tasks && planData.tasks.length > 0) {
      ...
      planData.tasks.forEach((task, index) => {
        ...
        if (task.dependencies) tasksSection += `  - **Dependencies**: ${task.dependencies}\n`;
        if (task.effort) tasksSection += `  - **Estimated Effort**: ${task.effort}\n`;
        tasksSection += '\n';
      });
      content = content.replace(/## Tasks Breakdown[\s\S]*?## Progress Tracking/, tasksSection + '## Progress Tracking');
    }
  ...
  async generateProjectPlan(projectData) {
    const planData = {
      ...
      tasks: projectData.features?.map((feature, index) => ({
        description: `Implement ${feature}`,
        effort: '2-4 hours',
        dependencies: index > 0 ? `TASK-${String(index).padStart(3, '0')}` : 'None'
      })) || []
    };
  ...
  async analyzeProjectProgress() {
    ...
    const analysis = {
      ...
      activePlans: Array.from(this.system.plans.values()).filter(p => p.status === 'active').length,
      completedPlans: Array.from(this.system.plans.values()).filter(p => p.status === 'completed').length,
      totalTasks: this.system.tasks.size,
      completedTasks: Array.from(this.system.tasks.values()).filter(t => t.completed).length,
      overallProgress: this.system.metrics.overallProgress,
      planDetails: []
    };

    for (const plan of this.system.plans.values()) {
      const planTasks = Array.from(this.system.tasks.values()).filter(t => t.planId === plan.id);
      const completedPlanTasks = planTasks.filter(t => t.completed);
      const planProgress = planTasks.length > 0 ? Math.round((completedPlanTasks.length / planTasks.length) * 100) : 0;
  ...
  async autoCompleteFinishedPlans() {
    ...
    for (const plan of this.system.plans.values()) {
      if (plan.status === 'active') {
        ...
        if (planTasks.length > 0 && completedTasks.length === planTasks.length) {
  ...
  async setupAutomation() {
    ...
    try {
      ...
      packageJson.scripts['plan:auto-complete'] = 'node planning-utils.js auto-complete';
