...
describe('GET /api/admin/bootstrap', () => {
  it('should return unified admin bootstrap payload', async () => {
    ...
    assert.strictEqual(response.body.aggregateLists[0].status.exists, true);
    assert.strictEqual(response.body.aggregateLists[0].status.revealed, true);
    assert.strictEqual(
      Object.hasOwn(response.body.aggregateLists[0], 'userListsVisible'),
      false
    );
