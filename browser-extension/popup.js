...
async function loadLists() {
  ...
  try {
    ...
    if (listNames.length === 0) {
      statusEl.innerHTML = '<div class="status error">No lists found. Create a list in SuShe Online first!</div>';
      return;
    }
...
async function refreshLists() {
  ...
  chrome.runtime.sendMessage({ action: 'refreshLists' }, (response) => {
    btn.disabled = false;
