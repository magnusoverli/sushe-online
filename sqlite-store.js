  ...
  if (userCount === 0 && (fs.existsSync(oldUsersFile) || fs.existsSync(oldListsFile))) {
    ...
    if (fs.existsSync(oldUsersFile)) {
      for (const doc of readNeDB(oldUsersFile)) {
        try { usersStore.insert(doc, () => {}); } catch (e) { console.error('Failed to insert user doc', e); }
      }
    }
    if (fs.existsSync(oldListsFile)) {
      for (const doc of readNeDB(oldListsFile)) {
        try { listsStore.insert(doc, () => {}); } catch (e) { console.error('Failed to insert list doc', e); }
      }
    }
