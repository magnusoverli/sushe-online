...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Account Settings - SuShe Online</title>
  <link href="/styles/output.css" rel="stylesheet">
  <link href="/styles/spotify-app.css" rel="stylesheet">
</head>
<body class="bg-black text-gray-200 overflow-y-auto">
  <div class="min-h-screen bg-gradient-to-br from-black via-gray-900 to-black pb-12">
    ...
    <div class="max-w-4xl mx-auto p-6">
      ...
      ${req.flash('error').length ? `
        <div class="bg-red-900/20 border border-red-800 text-red-400 px-4 py-3 rounded mb-6">
          ${req.flash('error')[0]}
        </div>
      ` : ''}
      ${req.flash('success').length ? `
        <div class="bg-green-900/20 border border-green-800 text-green-400 px-4 py-3 rounded mb-6">
          ${req.flash('success')[0]}
        </div>
      ` : ''}
      
      <!-- Account Information Section -->
      <div class="bg-gray-900/50 backdrop-blur-sm border border-gray-800 rounded-lg p-6 mb-6">
        ...
        <div class="space-y-4">
          ...
          <div>
            ...
            <p class="text-white">${new Date(data.user.createdAt).toLocaleDateString('en-US', { 
              year: 'numeric', 
              month: 'long', 
              day: 'numeric' 
            })}</p>
          ...
          <div class="pt-4 border-t border-gray-800">
            ...
            <p class="text-white">${data.stats.listCount} lists • ${data.stats.totalAlbums} total albums</p>
          </div>
        </div>
      </div>
      ...
      <!-- Request Admin Access Section (only show if not already admin) -->
      ${(!data.user.role || data.user.role !== 'admin') ? `
        <div class="bg-gray-900/50 backdrop-blur-sm border border-gray-800 rounded-lg p-6 mb-6">
          <h2 class="text-xl font-semibold text-white mb-4">Request Admin Access</h2>
          ...
          <form method="post" action="/account/request-admin" class="space-y-4">
            <div>
      ...
      ${data.user.role === 'admin' ? `
        <div class="bg-gray-900/50 backdrop-blur-sm border border-red-900 rounded-lg p-6">
          <h2 class="text-xl font-semibold text-red-500 mb-4">Administrator Access</h2>
          
          <div class="space-y-3 mb-6">
            <p class="text-gray-300">You have administrator privileges.</p>
            ${data.user.adminGrantedAt ? `
              <div class="text-sm text-gray-400">
                <p>Granted: ${new Date(data.user.adminGrantedAt).toLocaleString()}</p>
                ${data.user.adminGrantReason ? `<p>Reason: ${data.user.adminGrantReason}</p>` : ''}
              </div>
            ` : ''}
          </div>
          ...
          <a 
            href="/admin" 
            class="inline-block bg-red-600 hover:bg-red-700 text-white font-semibold py-2 px-6 rounded transition duration-200"
          >
